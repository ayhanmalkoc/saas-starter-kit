...
jest.mock('@/lib/guards/team-api-key', () => ({
  throwIfNoAccessToApiKey: jest.fn(),
}));
...
describe('/api/teams/[slug]/api-keys/[apiKeyId]', () => {
  ...
  it('returns access and role errors', async () => {
    ...
    await handler(
      { method: 'DELETE', query: { apiKeyId: 'key-1' } } as any,
      res1
    );
  ...
  it('returns api key access guard errors', async () => {
    (throwIfNoAccessToApiKey as jest.Mock).mockRejectedValueOnce({
      status: 404,
      message: 'API key not found',
    });
    const res = createRes();
    await handler(
      { method: 'DELETE', query: { apiKeyId: 'key-404' } } as any,
      res
    );

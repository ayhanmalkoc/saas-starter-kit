...
describe('/api/teams/[slug]/invitations', () => {
  beforeEach(() => {
    ...
    (getSession as jest.Mock).mockResolvedValue({
      user: { id: 'user-2', email: 'member@corp.com' },
    });
  });
  ...
  it('returns access errors and not-allowed errors', async () => {
    ...
    await handler(
      { method: 'GET', query: { sentViaEmail: 'true' } } as any,
      res1
    );
  ...
  it('POST rejects duplicate invitation', async () => {
    ...
    await handler(
      {
        method: 'POST',
        body: { sentViaEmail: true, email: 'a@corp.com', role: 'ADMIN' },
      } as any,
      res
    );
  ...
  it('POST creates invitation and records audit/event/metric', async () => {
    ...
    expect(sendEvent).toHaveBeenCalledWith('team-1', 'invitation.created', {
      id: 'inv-1',
      sentViaEmail: true,
    });
  ...
  it('PUT accepts invitation', async () => {
    ...
    await handler({ method: 'PUT', body: { inviteToken: 'tok' } } as any, res);
    expect(res.statusCode).toBe(204);
    expect(sendEvent).toHaveBeenCalledWith('team-1', 'member.created', {
      id: 'tm-1',
    });
